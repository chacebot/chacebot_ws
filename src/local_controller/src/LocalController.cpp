...
void LocalController::eventHandler() {
  readSerial();
  command_msg.steering =
      message.STATUS == SafetyStatus::is_enabled ? message.STEERING : 0;
  command_msg.drive =
      message.STATUS == SafetyStatus::is_enabled ? message.DRIVE : 0;
  command_msg.status =
      message.STATUS == SafetyStatus::is_enabled ? true : false;
}
